...
import { NodeType, AttributeType, EdgeType } from '../utils/umlConstants';
...
const UmlPromptPage: React.FC<UmlPromptPageProps> = ({
  ...
  const parsePrompt = (promptText: string): { action: string; details: any } | null => {
    // Palabras clave para detectar acciones
    const createClassKeywords = ['crear clase', 'nueva clase', 'agrega clase', 'crea una clase'];
    const editClassKeywords = ['editar clase', 'modificar clase', 'cambiar clase', 'actualizar clase'];
    const createRelationKeywords = ['crear relación', 'nueva relación', 'agregar relación', 'conectar'];
    ...
    if (createClassKeywords.some(keyword => promptLower.includes(keyword))) {
      ...
      if (classNameMatch && classNameMatch[1]) {
        ...
        const attrRegex = /(?:con|agregar|crear)\s+(?:atributo|campo)\s+(\w+)\s+(?:tipo|de tipo)?\s+(\w+)/gi;
        let attrMatch;
    ...
    if (createRelationKeywords.some(keyword => promptLower.includes(keyword))) {
      const sourceMatch = promptText.match(/entre\s+(\w+)\s+y/i) || 
                         promptText.match(/desde\s+(\w+)\s+hasta/i) ||
                         promptText.match(/de\s+(\w+)\s+a/i);
                         
      const targetMatch = promptText.match(/y\s+(\w+)/i) || 
      ...
      if (sourceMatch && targetMatch) {
        ...
        let tipo: 'asociacion' | 'agregacion' | 'composicion' | 'herencia' | 'dependencia' = 'asociacion';
        ...
        if (promptLower.includes('muchos a muchos') || promptLower.includes('n a n') || promptLower.includes('n:n')) {
          multiplicidadOrigen = "*";
          multiplicidadDestino = "*";
        } else if (promptLower.includes('uno a muchos') || promptLower.includes('1 a n') || promptLower.includes('1:n')) {
          multiplicidadOrigen = "1";
          multiplicidadDestino = "*";
        } else if (promptLower.includes('muchos a uno') || promptLower.includes('n a 1') || promptLower.includes('n:1')) {
          multiplicidadOrigen = "*";
  ...
  const executeCommand = () => {
    ...
    try {
      ...
      switch (parsedResult.action) {
        ...
        case 'createRelation': {
          const { sourceClass, targetClass, tipo, multiplicidadOrigen, multiplicidadDestino } = parsedResult.details;
          
          // Encontrar los nodos por nombre
          const sourceNode = existingNodes.find(node => 
            node.label.toLowerCase() === sourceClass.toLowerCase()
          );
          
          const targetNode = existingNodes.find(node => 
            node.label.toLowerCase() === targetClass.toLowerCase()
          );
          
          if (!sourceNode || !targetNode) {
            setFeedback(`Error: No pude encontrar una o ambas clases (${sourceClass}, ${targetClass}).`);
            break;
          }
        ...
        case 'editClass': {
          ...
          const nodeToUpdate = existingNodes.find(node => 
            node.label.toLowerCase() === className.toLowerCase()
          );
      ...
      <div className="uml-prompt-container">
        ...
        <div className="uml-prompt-body">
          <p className="instructions">
            ...
            <br />
            "Crea una clase llamada Usuario con atributo nombre tipo String y atributo edad tipo Integer"
            <br />
