 ...
 */
function convertOpenAIToUMLConstants(openaiData: OpenAIResponse): { nodes: NodeType[]; edges: EdgeType[] } {
  ...
  console.log(`📊 Relaciones originales: ${openaiData.relationships.length}, únicas: ${uniqueRelationships.length}`);
  ...
  const intermediateEntities = openaiData.classes.filter(cls => {
    ...
    return name.startsWith('detalle') || 
  ...
  intermediateEntities.forEach(intermediateEntity => {
    ...
    const connectionsToEntity = uniqueRelationships.filter(rel => 
      rel.target === entityName || rel.source === entityName
    );
    ...
    if (connectionsToEntity.length === 1) {
      ...
      const connectedEntity = existingConnection.source === entityName ? 
        existingConnection.target : existingConnection.source;
      ...
      const commonEntityPatterns = [
        'producto', 'item', 'articulo', 'elemento', 'objeto',
        'usuario', 'cliente', 'persona', 'agente',
        'servicio', 'tarea', 'actividad', 'operacion',
        'documento', 'archivo', 'registro', 'entrada',
        'categoria', 'tipo', 'clase', 'grupo'
      ];
      
      // Buscar por patrones comunes en las entidades disponibles
      for (const pattern of commonEntityPatterns) {
        const candidate = openaiData.classes.find(cls => 
      ...
      if (inferredEntity && openaiData.classes.some(cls => cls.name === inferredEntity)) {
        ...
        const relationExists = uniqueRelationships.some(rel => 
          (rel.source === inferredEntity && rel.target === entityName) ||
          (rel.source === entityName && rel.target === inferredEntity)
        );
        
        if (!relationExists) {
          console.log(`   ✅ Agregando relación inferida: ${inferredEntity} → ${entityName} (inferencia universal)`);
  ...
  nodes.forEach(candidateNode => {
    ...
    console.log(`   Relaciones conectadas: ${allConnectedRels.length} (${incomingRels.length} entrantes, ${outgoingRels.length} salientes)`);
    ...
    const isIntermediaEntity = ['intermedia', 'relacion', 'inscripcion', 'asociacion', 'union'].some(word => 
    ...
    console.log(`   🎯 Entidades únicas conectadas: ${entityArray.length} (${entityArray.join(', ')})`);
    
    // PASO 5: Debe conectar exactamente 2 entidades principales
    if (entityArray.length !== 2) {
      console.log(`   ❌ Rechazado: conecta ${entityArray.length} entidades únicas (debe ser exactamente 2)`);
      return;
    }
    ...
    const entityAIsIntermediate = entityA.toLowerCase().startsWith('detalle') || 
      ['intermedia', 'relacion', 'inscripcion'].some(word => entityA.toLowerCase().includes(word));
    const entityBIsIntermediate = entityB.toLowerCase().startsWith('detalle') || 
    ...
    if (!entityIdA || !entityIdB) {
      console.log(`   ❌ Error: IDs no encontrados para ${entityA} (${entityIdA}) o ${entityB} (${entityIdB})`);
      return;
    }
  ...
  nodes.filter(n => n.asociativa && n.relaciona).forEach(tableNode => {
  ...
  uniqueRelationships.forEach((rel, idx) => {
    ...
    console.log(`🔗 Relación ${idx + 1}: ${rel.source} → ${rel.target} (${rel.type}) mult: "${rel.multiplicity || 'sin mult'}"`);
    ...
    if (!sourceId || !targetId) {
      console.log(`  ❌ Error: No se encontraron IDs para ${rel.source} (${sourceId}) → ${rel.target} (${targetId})`);
      return;
    }
    ...
    edges.push(edge);
    console.log(`  ✅ Relación directa agregada: ${rel.source} → ${rel.target} (${edge.tipo}) [${origen}:${destino}]`);
  });
 ...
 */
function mapOpenAITypeToUMLType(openaiType: string): 'Integer' | 'Float' | 'Boolean' | 'Date' | 'String' {
 ...
 */
function mapOpenAIRelationType(openaiType: string): 'asociacion' | 'agregacion' | 'composicion' | 'herencia' | 'dependencia' {
...
function correctRelationshipDirection(rel: OpenAIRelationship): OpenAIRelationship {
  ...
  if (source.length < target.length && target.includes(source)) {
    console.log(`   🔄 Heurística especificidad: ${rel.source} → ${rel.target} INVERTIDA (entidad general → específica)`);
    return {
      ...rel,
      source: rel.target,
      target: rel.source
  ...
  if (!sourceIsPlural && targetIsPlural) {
    console.log(`   🔄 Heurística cardinalidad: ${rel.source} → ${rel.target} INVERTIDA (singular → plural)`);
    return {
      ...rel,
      source: rel.target,
      target: rel.source
...
- Analizar cuidadosamente las líneas y multiplicidades visibles en el diagrama`;
  ...
  try {
    ...
    parsedData.classes?.forEach((cls: any, index: number) => {
      ...
      console.log(`     Atributos: [${cls.attributes?.map((a: any) => `${a.name}:${a.type}(${a.visibility || 'sin-scope'})`).join(', ') || 'ninguno'}]`);
    });
    ...
    if (relationshipTypes.size === 1 && relationshipTypes.has('association')) {
      ...
      console.log(`     Si el diagrama tiene herencia/composición/agregación, revisar detección visual.`);
    }
    ...
    const tablesWithDetalle = parsedData.classes?.filter((cls: any) => 
      cls.name.toLowerCase().includes('detalle') || 
      cls.name.toLowerCase().includes('intermedia') ||
      cls.name.toLowerCase().includes('inscripcion')
    ) || [];
    console.log(`  Posibles tablas intermedias: ${tablesWithDetalle.map((t: any) => t.name).join(', ') || 'ninguna'}`);
    ...
    expectedCommonRelations.forEach(expected => {
      const hasRelation = parsedData.relationships?.some((rel: any) => 
      ...
      if (hasActor && hasAction && !hasRelation) {
        console.log(`  🚨 RELACIÓN FALTANTE DETECTADA: ${expected.actor || expected.classifier} ↔ ${expected.action || expected.classified}`);
      }
    });
